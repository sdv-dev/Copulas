...
class TestGaussianCopula(TestCase):
    ...
    def setUp(self):
        ...
        self.structured_data = pd.DataFrame({
            ...
            'bimodal': np.random.normal(size=1000) * mask
            + 10.0 * np.random.normal(size=1000) * (1.0 - mask)
        })
    ...
    def test_fit_distribution_auto(self):
        ...
        assert get_qualified_name(copula.distribs["truncnorm"].__class__) \
            == 'copulas.univariate.truncated_gaussian.TruncatedGaussian'
        assert get_qualified_name(copula.distribs["bimodal"].__class__) \
            == 'copulas.univariate.gaussian_kde.GaussianKDE'

    def test_fit_distribution_partial_auto(self):
        """On fit, the distributions should be automatically selected if not specified."""
        copula = GaussianMultivariate(distribution={
