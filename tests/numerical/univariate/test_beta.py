...
class TestBetaUnivariate(TestCase):
    ...
    def test_fit_and_sample(self):
        np.random.seed(42)
        self._test_fit_and_sample(a=1.0, b=1.0, loc=5.0, scale=2.0)
        self._test_fit_and_sample(a=1.0, b=2.0, loc=5.0, scale=1.0)
        self._test_fit_and_sample(a=2.0, b=1.0, loc=5.0, scale=2.0)
        self._test_fit_and_sample(a=2.0, b=2.0, loc=-1.0, scale=7.0)

    def _test_fit_and_sample(self, a, b, loc, scale, N=10000):
        """
        Sample N data points from the beta distribution with the given parameters
        ...
        data = beta.rvs(a, b, loc, scale, size=N)
