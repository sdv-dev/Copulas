...
class CopulaModel(object):
	...
	Attributes:
		...
		c_type: A string indicating the type of copula models, can be 'gaussian','frank','clayton','gumbel','cvine','dvine','t','PCA'
	...
	def _fit_model(self):
		...
		if 'vine' not in self.c_type:
			if self.n_var!=2:
				raise Exception('bivariate copula only support 2 variables,' + str(self.u_matrix.shape[1])+' provided')
			else:
