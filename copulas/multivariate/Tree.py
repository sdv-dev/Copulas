	...
	def _get_constraints(self):
		...
		for k in range(len(self.edge_set)):
			for i in range(len(self.edge_set)):
				...
				if k!=i and self._check_adjacency(self.edge_set[k],self.edge_set[i]):
...
class CTree(Tree):
	...
	def _build_first_tree(self):
		...
		for itr in range(1,self.n_nodes):
			...
			name,param = copula.select_copula(U[:,0],U[:,ind])
	...
	def _build_kth_tree(self):
		...
		for itr in range(self.n_nodes-1):
			...
			U1,U2 = self.prev_T.edge_set[anchor].U,self.prev_T.edge_set[right].U
			name,param=copula.select_copula(U1,U2)
			[ed1,ed2,ing]=self.identify_eds_ing(self.prev_T.edge_set[anchor],self.prev_T.edge_set[right])
...
class DTree(Tree):
	...
	def _build_first_tree(self):
		...
		for k in range(self.n_nodes-1):
			name,param = copula.select_copula(self.u_matrix[:,T1[k]],self.u_matrix[:,T1[k+1]])
			cop = copula.Copula(cname=name)
...
class RTree(Tree):
	...
	def _build_first_tree(self):
		...
		while len(X)!=self.n_nodes:
			...
			name,param=copula.select_copula(self.u_matrix[:,edge[0]],self.u_matrix[:,edge[1]])
